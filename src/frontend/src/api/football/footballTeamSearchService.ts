...
import type { FootballSeasonDto } from './footballSeasonService';
...
export const footballTeamSearchService = {
  /**
  ...
  searchTeams: async (query: string, page: number): Promise<SearchResult> => {
    try {
      ...
      let teams: DropdownOption[] = response.data.map((team: FootballTeam) => ({
        id: team.id,
        name: team.name,
      }));
...
export const footballSeasonSearchService = {
  /**
  ...
  searchSeasons: async (query: string, page: number): Promise<SearchResult> => {
    try {
      ...
      const { footballSeasonService } = await import('./footballSeasonService');
      ...
      let seasons: DropdownOption[] = response.data.map((season: FootballSeasonDto) => ({
        id: season.id,
        name: season.name,
      }));
...
export const footballTournamentSearchService = {
  /**
   * Search tournaments for dropdown (typically few, so we can load all and filter client-side
   * â€” same pattern as the season search service).
   */
  searchTournaments: async (query: string, page: number): Promise<SearchResult> => {
    try {
      ...
      const { footballTournamentService } = await import('./footballTournamentService');
