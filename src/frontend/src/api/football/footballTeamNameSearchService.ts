import type { ApiResponse, FootballTeamNameResult } from '../../types/football/footballTypes';
...
export const footballTeamNameSearchService = {
  /**
   * Get team names filtered by name filter (non-paginated backend call)
   */
  getTeamNames: async (nameFilter: string): Promise<ApiResponse<FootballTeamNameResult[]>> => {
    const searchParams = new URLSearchParams();
    if (nameFilter) {
      searchParams.append('nameFilter', nameFilter);
    }

    const url = `${API_URL}/FootballTeam/names${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
    ...
    const apiResponse: ApiResponse<FootballTeamNameResult[]> = await response.json();
  ...
  searchTeams: async (query: string, page: number): Promise<SearchResult> => {
    try {
      const response = await footballTeamNameSearchService.getTeamNames(query);
