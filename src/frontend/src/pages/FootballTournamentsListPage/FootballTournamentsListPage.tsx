...
import './FootballTournamentsListPage.scss';
...
function FootballTournamentsListPage() {
  ...
  const fetchTournaments = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await footballTournamentService.getAll(audience.teamCategory);
  ...
  const renderTournamentCard = (tournament: FootballTournamentDto) => {
    ...
    return (
      <div key={tournament.id} className="tournament-card">
        ...
        <nav className="tournament-card__links" aria-label={tournament.name}>
          <Link to={`/football/tournaments/${tournament.id}?tab=summary`} className="tournament-card__link">
            {t('tournaments.tabSummary', 'Yhteenveto')}
          </Link>
          <Link to={`/football/tournaments/${tournament.id}?tab=groups`} className="tournament-card__link">
            {t('tournaments.tabGroups', 'Lohkot')}
          </Link>
          <Link to={`/football/tournaments/${tournament.id}?tab=fixtures`} className="tournament-card__link">
            {t('tournaments.tabFixtures', 'Otteluohjelma')}
          </Link>
          <Link to={`/football/tournaments/${tournament.id}?tab=results`} className="tournament-card__link">
            {t('tournaments.tabResults', 'Tulokset')}
          </Link>
          <Link to={`/football/tournaments/${tournament.id}?tab=statistics`} className="tournament-card__link">
...
export default FootballTournamentsListPage;
