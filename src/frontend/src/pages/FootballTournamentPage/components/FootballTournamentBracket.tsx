...
import type {
  ...
  FootballPlayoffMatchDto,
  FootballPlayoffRoundDto,
  FootballPlayoffRoundKey,
  FootballPlayoffTeamDto
} from '../../../types/football/tournamentTypes';
import './FootballTournamentBracket.scss';

interface TournamentBracketProps {
  bracket: FootballPlayoffBracketDto;
  /**
   ...
   *  - 'public' (default): navigate to the public match detail page (`/football/match/{id}`).
   *  - 'admin': navigate to the admin match management view (`/admin/football/matches/manage/{id}`).
   */
...
const ROUND_DISPLAY_ORDER: FootballPlayoffRoundKey[] = [
  'QuarterFinal',
  'SemiFinal',
  'ThirdPlaceMatch',
  'Final'
...
interface TeamSlotProps {
  team: FootballPlayoffTeamDto | null;
  resolved: boolean;
  score: number | null;
  isWinner: boolean;
  tbdLabel: string;
...
interface MatchCardProps {
  match: FootballPlayoffMatchDto;
  roundKey: FootballPlayoffRoundKey;
...
export default function TournamentBracket({ bracket, compact = false, linkMode = 'public', adminReturnTo, onAssignTeams }: TournamentBracketProps): ReactElement {
  ...
  const orderedRounds = useMemo<FootballPlayoffRoundDto[]>(() => {
    const roundMap = new Map<FootballPlayoffRoundKey, FootballPlayoffRoundDto>();
    bracket.rounds.forEach((r) => {
      roundMap.set(r.round, r);
    });
    return ROUND_DISPLAY_ORDER
      .map((key) => roundMap.get(key))
      .filter((r): r is FootballPlayoffRoundDto => r !== undefined);
  }, [bracket.rounds]);

  const handleSelectMatch = (matchId: string): void => {
    if (linkMode === 'admin') {
      const base: string = `/admin/football/matches/manage/${matchId}`;
  ...
  const assignTeamsLabel = t('football.matches.assignTeams.action', 'Aseta joukkueet');
  ...
  const roundLabels: Record<FootballPlayoffRoundKey, string> = {
