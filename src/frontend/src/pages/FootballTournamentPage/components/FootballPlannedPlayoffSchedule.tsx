...
import './FootballPlannedPlayoffSchedule.scss';
...
export default function PlannedPlayoffSchedule({
  ...
  const orderedSlots = useMemo<PlayoffScheduleSlotDto[]>(() => {
    ...
    const ROUND_ORDER: Record<FootballPlayoffRoundKey, number> = {
      QuarterFinal: 1,
      SemiFinal: 2,
      ThirdPlaceMatch: 3,
      Final: 4,
  ...
  const tbd = t('football.tournaments.playoffSchedule.tbd', 'TBD');
  const tooltip = t(
    'football.tournaments.playoffSchedule.placeholderTooltip',
  ...
  const toggleLabel = expanded
    ? t('football.tournaments.playoffSchedule.toggleHide', 'Piilota pudotuspeliaikataulu')
    : t('football.tournaments.playoffSchedule.toggleShow', 'Näytä pudotuspeliaikataulu ({{count}})', {
        count: slotCount
      });
        ...
        <>
          <div className="planned-playoff-schedule__header">
            <span className="planned-playoff-schedule__title">
              {t('football.tournaments.playoffSchedule.title', 'Planned playoff matches')}
            </span>
            <span className="planned-playoff-schedule__hint">
              {t(
                'football.tournaments.playoffSchedule.hint',
...
function formatSlotLabel(slot: PlayoffScheduleSlotDto, t: (key: string, fallback: string) => string): string {
  switch (slot.round) {
    case 'QuarterFinal':
      return `${t('football.tournaments.playoffSchedule.rounds.qf', 'QF')}${slot.order + 1}`;
    case 'SemiFinal':
      return `${t('football.tournaments.playoffSchedule.rounds.sf', 'SF')}${slot.order + 1}`;
    case 'ThirdPlaceMatch':
      return t('football.tournaments.playoffSchedule.rounds.thirdPlace', '3rd place');
    case 'Final':
      return t('football.tournaments.playoffSchedule.rounds.final', 'Final');
    default:
