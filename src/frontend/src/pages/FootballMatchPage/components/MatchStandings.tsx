...
export default function MatchStandings({ match }: MatchStandingsProps) {
  ...
  const [seasonStats, setSeasonStats] = useState<FootballSeasonStatisticsSummaryDto | null>(null);
  ...
  const isTournament: boolean = useMemo(
    ...
    [match.competitionType, match.tournamentGroupId, match.tournamentStage],
  );
  ...
  useEffect(() => {
    ...
    const fetchSeasonStats = async () => {
      try {
        ...
        setStatsError(null);
        const data = await footballStatisticsService.getSeasonStatistics(match.competitionId);
        if (!cancelled) {
          setSeasonStats(data);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchTournament = async () => {
      try {
        ...
        const response = await footballTournamentService.getById(match.competitionId);
  ...
  useEffect(() => {
    ...
    const fetchBracket = async () => {
      try {
        ...
        const response = await footballTournamentService.getPlayoffBracket(match.competitionId);
        if (!cancelled && response.success && response.data) {
  ...
  if (isGroupStageMatch && match.tournamentGroupId) {
    ...
    if (bracketLoading && !bracket) {
      ...
      standingsOverride = (
        <div style={{ padding: '1.5rem', textAlign: 'center', color: '#6b7280' }}>
          {t('football.tournaments.bracket.empty', 'Pudotuspelikaaviota ei ole vielä luotu.')}
        </div>
      );
    }
  }
