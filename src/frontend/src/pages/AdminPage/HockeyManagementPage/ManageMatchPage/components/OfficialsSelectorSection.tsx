...
const OfficialsSelectorSection = ({
  ...
  return (
    <section
      ...
      aria-label={t('hockey.matches.manage.matchOfficials', 'Match officials')}
    >
      <div className="officials-selector-section__header">
        <h3 className="officials-selector-section__title">
          {t('hockey.matches.manage.matchOfficials', 'MATCH OFFICIALS')}
        </h3>
        ...
        {!disabled && (
          ...
          >
            ...
            {t('hockey.matches.manage.addReferee', 'Add referee')}
          </button>
        )}
      </div>
      ...
      {selectedOfficials.length === 0 ? (
        <div className="officials-selector-section__empty">
          {t('hockey.matches.manage.noOfficials', 'No officials assigned.')}
        </div>
      ) : (
          ...
          {selectedOfficials.map((refId, idx) => (
            <div className="officials-selector-section__row" key={`${idx}-${refId || 'empty'}`}>
              ...
              >
                <option value="">
                  {saving
                    ? t('hockey.matches.manage.saving', 'Saving...')
                    : t('hockey.matches.manage.selectReferee', 'SELECT REFEREE')}
                </option>
                {options.map(option => (
                  <option
                    key={option.id}
              ...
              {!disabled && (
                <button
                  ...
                  aria-label={t('hockey.matches.manage.removeReferee', 'Remove referee')}
                >
                  ×
                </button>
              )}
