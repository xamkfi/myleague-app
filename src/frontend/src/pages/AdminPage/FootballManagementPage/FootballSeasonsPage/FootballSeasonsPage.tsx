...
import './FootballSeasonsPage.scss';
...
import { FootballRosterImportModal } from './components/RosterImportModal';
...
const FootballSeasonsPage = () => {
  ...
  const [seasonToComplete, setSeasonToComplete] =
    useState<FootballSeasonDto | null>(null);
  ...
  const handleEditSeason = (seasonId: string): void => {
    navigate(`/admin/football/seasons/${seasonId}/edit`);
  };
   ...
   */
  const openCompleteSeasonModal = (season: FootballSeasonDto): void => {
  ...
  return (
    <PageTemplate title={t('football.seasons.title', 'Manage Seasons')}>
      <div className="football-seasons-container">
        <SeasonsPageHeader
...
export default FootballSeasonsPage;