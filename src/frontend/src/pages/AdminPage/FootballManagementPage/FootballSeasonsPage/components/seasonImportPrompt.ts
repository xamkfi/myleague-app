export const FOOTBALL_SEASON_IMPORT_AI_PROMPT: string = `You are converting a football league-season schedule (and optional team-roster sheets) into a single JSON file for the MyLeague season import feature. Output ONLY the JSON object — no markdown fences, no commentary, no trailing text.
...
# Schema (all field names case-sensitive)
{
  ...
  "season": {
    ...
    "defaultVenue"?: string,
    "numberOfHalves"?: number,                 // typical hobby: 2
    "halfDurationMinutes"?: number,             // typical hobby: 20
    "playersOnField"?: number,                  // typical hobby: 5
  ...
  "teams": [
    {
      ...
      "players"?: [
        { "firstName": string, "lastName": string, "jerseyNumber"?: number, "position"?: "Goalkeeper" | "Defender" | "Midfielder" | "Forward" }
      ]
    }
  ],
  "matches": [
...
export function buildFootballSeasonPromptFileName(): string {
  const now: Date = new Date();
  const pad = (n: number): string => n.toString().padStart(2, '0');
  return `myleague-football-season-import-prompt-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}.txt`;
}
