...
const FootballPlayersPage = () => {
  ...
  const [allPlayers, setAllPlayers] = useState<FootballPlayerDto[]>([]); // Cache for search
  ...
  const fetchAllPlayers = useCallback(async () => {
    try {
      ...
      try {
        ...
        const fallbackResponse = await footballPlayerService.getAll({
          page: 1,
          pageSize: 50,
        });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setError(t('football.players.errors.deleteFailed', 'Failed to delete player. Please try again.'));
      console.error(err);
    } finally {
      setIsDeleting(false);
    }
