...
interface TeamPlayersRowProps {
  ...
  isExpanded: boolean;
  isClosing: boolean;
  team?: FootballTeam; // Add team data as fallback
}
...
const TeamPlayersRow = ({ teamId, isExpanded, isClosing, team }: TeamPlayersRowProps) => {
  ...
  const fetchPlayers = async () => {
    try {
      ...
      const playersData = await footballPlayerService.getByTeamId(teamId);
      ...
      <div className="team-players-container">
        ...
        {!loading && !error && playerCount === 0 && (
          <div className="no-players">
            <p>{t('football.teams.noPlayersInTeam', 'This team has no players assigned yet.')}</p>
            <p className="help-text">
              {t('football.teams.addPlayersHelp', 'Use the edit button to manage team roster and add players.')}
            </p>
          </div>
        )}
            ...
            {playerPositions.map((pos, key) => (
              <div key={key} className="admin-roster-group">
                <div className="admin-roster-position-header">
                  {t(`football.positions.${pos.toLowerCase()}`, pos)}
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="col-name">
                            {'id' in player ? (
                              <PlayerLink sport="football" playerId={player.id}>
                                {playerName}
                              </PlayerLink>
                            ) : (
                              playerName
