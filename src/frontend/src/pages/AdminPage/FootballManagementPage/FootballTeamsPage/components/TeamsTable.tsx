...
const TeamsTable = ({
  ...
  useEffect(() => {
    const fetchDivisions = async () => {
      ...
      setDivisions(tempDivisions.data.filter((division) => division.sportType === SportsCategory.Football));
    };
    fetchDivisions();
  }, []);
    ...
    <>
      ...
      <div className="admin-table__wrapper">
        {loading ? (
          ...
          <div className="admin-table__empty">
            {t('football.teams.noTeams', 'No teams found')}
          </div>
        ) : (
              ...
              <tr>
                <th className="admin-table__checkbox-col">
                  <input
                    ...
                    title={t('football.teams.selectAll', 'Select all teams')}
                  />
                </th>
                <th>{t('football.teams.table.name', 'Team Name')}</th>
                <th>{t('football.teams.table.club', 'Club')}</th>
                <th>{t('football.teams.table.division', 'Division')}</th>
                <th>{t('football.teams.table.homeArena', 'Home Arena')}</th>
                <th>{t('football.teams.table.activeMembers', 'Active Members')}</th>
                <th className="admin-table__actions-col">{t('football.teams.table.actions', 'Actions')}</th>
              </tr>
            </thead>
            <tbody>
              {teams.map((team) => (
                  ...
                  >
                        ...
                        <div className="team-info">
                          ...
                          <div className="jersey-colors">
                            <span
                              ...
                              title={`${t('football.teams.primary', 'Primary')}: ${team.primaryJerseyColor}`}
                            ></span>
                            {team.secondaryJerseyColor && (
                              <span
                                className="color-indicator secondary"
                                style={{ backgroundColor: team.secondaryJerseyColor }}
                                title={`${t('football.teams.secondary', 'Secondary')}: ${team.secondaryJerseyColor}`}
                              ></span>
                            )}
                          </div>
                        </div>
                    ...
                    <td>
                      <span className={`admin-badge ${team.hasActiveMembers ? 'admin-badge--active' : 'admin-badge--inactive'}`}>
                        {team.hasActiveMembers
                          ? t('football.teams.hasMembers', 'Yes')
                          : t('football.teams.noMembers', 'No')
                        }
                      </span>
                    </td>
                    <td className="admin-table__actions-col" onClick={(e) => e.stopPropagation()}>
                      <ActionsDropdown
                        actions={[
                          {
                            label: t('football.teams.editTeamInfo', 'Edit Team Information'),
                            onClick: () => onEdit(team.id),
                          },
                          {
                            label: t('football.teams.editRoster', 'Edit Roster'),
                        ...
                        ariaLabel={t('football.teams.actions.menu', 'Team actions menu')}
                      />
                    </td>
                  </tr>
