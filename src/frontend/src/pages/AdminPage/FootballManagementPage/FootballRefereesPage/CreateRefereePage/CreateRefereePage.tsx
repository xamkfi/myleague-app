...
const CreateRefereePage = () => {
  ...
  useEffect(() => {
    const fetchExistingReferees = async () => {
      try {
        const refereesResponse = await footballRefereeService.getAll({ pageSize: 50 });
        const ids = new Set(
  ...
  const handleCreateReferee = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const person of selectedPersonsList) {
        try {
          await footballRefereeService.create({ 
            PersonId: person.id,
      ...
      const message = successCount === 1
        ? t('football.referees.refereeCreated', '{{count}} referee created successfully!', { count: successCount })
        : t('football.referees.refereesCreated', '{{count}} referees created successfully!', { count: successCount });
  ...
  if (initialLoading) {
    return (
      <PageTemplate title={t('football.referees.createNew', 'Create New Referee')}>
  ...
  return (
    <PageTemplate title={
      showCreateForm 
        ? t('football.referees.createRefereeForMultiple', 'Create Referees for {{count}} person(s)', { count: selectedPersonIds.size })
        : t('football.referees.createNew', 'CREATE REFEREE FROM AVAILABLE PERSONS')
    }>
        ...
        {!showCreateForm ? (
          <>
            ...
            <div className="search-header">
              <div className="search-container">
                <input
                  type="text"
                  placeholder={t('football.referees.searchPersons', 'Search available persons...')}
                  value={searchTerm}
                  onChange={(e) => {
              ...
              <button className="create-person-link" onClick={() => navigate('/admin/persons/new')}>
                ...
                {t('football.referees.createNewPerson', 'Create new person')}
              </button>
            </div>
            ...
            <div className="selection-controls">
              <div className="selection-info">
                <span className="selected-count">
                  {t('football.referees.selectedCount', '{{count}} selected', { count: selectedPersonIds.size })}
                </span>
              ...
              <div className="proceed-action">
                ...
                >
                  {t('football.referees.createReferees', 'Create referee(s) ({{count}})', { count: selectedPersonIds.size })}
                </button>
              </div>
            </div>
            ...
            <div className={`persons-table-wrapper${loading ? ' is-loading' : ''}`}>
              {filteredPersons.length === 0 && !loading ? (
                <div className="no-persons">
                  <p>{searchTerm ? 
                    t('football.referees.noPersonsFound', 'No persons found matching your search') :
                    t('football.referees.noPersonsAvailable', 'No available persons to convert to referees. All persons are already referees.')
                  }</p>
                </div>
              ) : (
          ...
          <form onSubmit={handleCreateReferee} className="create-referee-form">
            ...
            <div className="selected-persons-list">
              <h3>{t('football.referees.selectedPersons', 'Selected persons:')}</h3>
              <ul>
            ...
            <div className="form-actions">
              ...
              >
                {creating ? 
                  ...
                  t('football.referees.createReferee', 'Create referee')
                }
              </button>
            </div>
          </form>
