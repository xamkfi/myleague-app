...
const RefereesTable = ({
  ...
  if (safeReferees.length === 0) {
    return <div className="admin-table__empty">{t('football.referees.noReferees', 'No referees found.')}</div>;
  }
        ...
        <tr>
          <th className="admin-table__checkbox-col">
            <input
              ...
              title={t('football.referees.selectAll', 'Select all referees')}
            />
          </th>
          <th>{t('football.referees.table.name', 'Name')}</th>
          <th>{t('football.referees.table.status', 'Status')}</th>
          <th>{t('football.referees.table.licenseExpiry', 'License Expiry')}</th>
          <th>{t('football.referees.table.matchesOfficiated', 'Matches')}</th>
          <th className="admin-table__actions-col">{t('football.referees.table.actions', 'Actions')}</th>
        </tr>
      </thead>
      <tbody>
            ...
            >
                ...
                <div>
                  ...
                  {referee.licenseExpiryDate && (
                    <span className={getLicenseBadgeClass(licenseStatus)}>
                      {licenseStatus === 'expired' && t('football.referees.license.expired', 'Expired')}
                      {licenseStatus === 'expiring' && t('football.referees.license.expiring', 'Expiring Soon')}
                      {licenseStatus === 'valid' && t('football.referees.license.valid', 'Valid')}
                    </span>
                  )}
                </div>
              </td>
              ...
              <td className="admin-table__actions-col">
                <ActionsDropdown
                  actions={[
                    {
                      ...
                      onClick: () => navigate(`/admin/football/referees/${referee.id}/edit`),
                    },
                    {
                  ...
                  ariaLabel={t('football.referees.actions.menu', 'Referee actions menu')}
                />
              </td>
            </tr>
          );
