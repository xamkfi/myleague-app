      ...
      <div className="live-match-info">
        ...
        <div className="status-controls">
          {currentMatch.status === 'Completed' ? (
            <>
              <span className="match-status">🏁 {t('football.matches.manage.finished', 'FINISHED')}</span>
              <button
                onClick={onReopen}
                className="reopen-match-button"
                title={t('football.matches.manage.reopenMatchTitle', 'Reopen this match for editing')}
              >
                🔓 {t('football.matches.manage.reopenMatch', 'Open match')}
              </button>
