...
interface TournamentMatchesTabProps {
   ...
   */
  onTournamentUpdated?: (tournament: FootballTournamentDto) => void;
}
...
const TournamentMatchesTab = ({ tournament, onTournamentUpdated }: TournamentMatchesTabProps): ReactElement => {
  ...
  const handleSchedulePersisted = useCallback((updated: FootballTournamentDto): void => {
  ...
  const manageAllPath: string = useMemo(
    () => `/admin/football/tournaments/matches?competitionId=${tournamentId}`,
    [tournamentId]
  );
  ...
  const tournamentEditReturnTo: string = useMemo(
    () => `/admin/football/tournaments/${tournamentId}/edit?tab=matches`,
    [tournamentId]
  );

  const createMatchPath: string = useMemo(
    () => `/admin/football/tournaments/matches/create?competitionId=${tournamentId}&returnTo=${encodeURIComponent(tournamentEditReturnTo)}`,
  ...
  const handleOpenMatch = useCallback(
    (match: FootballMatchDto): void => {
      navigate(`/admin/football/matches/manage/${match.id}?returnTo=${encodeURIComponent(tournamentEditReturnTo)}`);
    },
    ...
    <div className="tmt">
      <div className="tmt__action-row">
        <button type="button" className="tmt__btn" onClick={handleManageAll}>
          ...
          {t('football.tournaments.actions.manageMatches', 'Manage tournament matches')}
        </button>
        <button type="button" className="tmt__btn" onClick={handleCreateNew}>
          <i className="fas fa-plus" aria-hidden="true"></i>
          {t('football.tournaments.matchesTab.createNew', 'Create new match')}
        </button>
      </div>
      ...
      {isEmpty ? (
        <div className="tmt__empty">
          ...
          <p>
            {t(
              'football.tournaments.matchesTab.empty',
              'No matches have been added to this tournament yet.'
            )}
          </p>
          <button type="button" className="tmt__btn" onClick={handleCreateNew}>
            <i className="fas fa-plus" aria-hidden="true"></i>{' '}
            {t('football.tournaments.matchesTab.createFirst', 'Create the first match')}
          </button>
        </div>
      ) : (
