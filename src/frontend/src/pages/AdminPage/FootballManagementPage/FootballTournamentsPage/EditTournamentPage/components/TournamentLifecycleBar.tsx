 ...
 * (`footballTournamentService.delete`) and must navigate the user away from
...
interface TournamentLifecycleBarProps {
  ...
  matches: ReadonlyArray<FootballMatchDto>;
...
const getStatusLabel = (
  ...
  switch (status) {
    case 'Draft':
      return t('football.tournaments.status.draft', 'Luonnos');
    case 'GroupStage':
      return t('football.tournaments.status.groupStage', 'Alkulohkot');
    case 'PlayoffStage':
      return t('football.tournaments.status.playoffStage', 'Pudotuspelit');
    case 'Completed':
      return t('football.tournaments.status.completed', 'Päättynyt');
    case 'Cancelled':
      return t('football.tournaments.status.cancelled', 'Peruttu');
    default:
      return status;
  }
};
...
export const TournamentLifecycleBar = ({
  ...
  const groupStageMatches: ReadonlyArray<FootballMatchDto> = useMemo(
  ...
  const startGroupStageEligibility: ActionEligibility = useMemo<ActionEligibility>(() => {
    ...
    const prereqs: LifecyclePrerequisite[] = [
      {
        label: t(
          'football.tournaments.lifecycle.prereq.tournamentInDraft',
      ...
      {
        label: t(
          'football.tournaments.lifecycle.prereq.hasAtLeastOneGroup',
    ...
    const disabledReason: string | null = enabled
      ? null
      : t(
          'football.tournaments.lifecycle.disabled.noGroups',
    ...
    return {
      ...
      tooltip: enabled
        ? t(
            'football.tournaments.lifecycle.tooltip.startGroupStage',
  ...
  const startPlayoffEligibility: ActionEligibility = useMemo<ActionEligibility>(() => {
    ...
    const prereqs: LifecyclePrerequisite[] = [
      {
        label: t(
          'football.tournaments.lifecycle.prereq.hasPlayoffStageEnabled',
      ...
      {
        label: t(
          'football.tournaments.lifecycle.prereq.supportedPlayoffSize',
      ...
      {
        label: t(
          'football.tournaments.lifecycle.prereq.allGroupStageMatchesDone',
    ...
    if (matchesLoading) {
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.loadingMatches',
        'Ladataan otteluiden tila...'
      );
    } else if (!playoffSizeSupported) {
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.unsupportedPlayoffSize',
      ...
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.noGroupStageMatches',
        'Yhtään alkulohkon ottelua ei ole vielä luotu.'
      );
    } else if (!allGroupStageDone) {
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.groupStageInProgress',
    ...
    return {
      ...
      tooltip: enabled
        ? t(
            'football.tournaments.lifecycle.tooltip.startPlayoffStage',
  ...
  const completeEligibility: ActionEligibility = useMemo<ActionEligibility>(() => {
    ...
    const prereqs: LifecyclePrerequisite[] = [
      {
        label: t(
          'football.tournaments.lifecycle.prereq.tournamentIsRunning',
      ...
      {
        label: t(
          'football.tournaments.lifecycle.prereq.allMatchesDone',
    ...
    if (matchesLoading) {
      ...
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.noMatches',
        'Turnauksessa ei ole vielä otteluita.'
      );
    } else if (!allDone) {
      disabledReason = t(
        'football.tournaments.lifecycle.disabled.matchesInProgress',
    ...
    return {
      ...
      tooltip: enabled
        ? t(
            'football.tournaments.lifecycle.tooltip.complete',
  ...
  const cancelEligibility: ActionEligibility = useMemo<ActionEligibility>(() => {
    ...
    return {
      ...
      enabled: visible,
      tooltip: t(
        'football.tournaments.lifecycle.tooltip.cancel',
  ...
  const renderPrimaryButton = (): ReactElement | null => {
    ...
    const label: string =
      primaryAction === 'startGroupStage'
        ? t('football.tournaments.lifecycle.startGroupStage', 'Aloita Alkulohkot')
        : primaryAction === 'startPlayoffStage'
          ? t('football.tournaments.lifecycle.startPlayoffStage', 'Aloita Pudotuspelit')
          : t('football.tournaments.lifecycle.complete', 'Päätä Turnaus');
  ...
  const modalConfig: {
    ...
    switch (activeAction) {
      case 'startGroupStage':
        return {
          title: t(
            'football.tournaments.lifecycle.confirm.startGroupStage.title',
            'Aloita alkulohkot'
          ),
          description: t(
            'football.tournaments.lifecycle.confirm.startGroupStage.description',
            'Merkitsee turnauksen aktiiviseksi. Tämän jälkeen aikataulu ja otteluiden tiedot näkyvät yleisösivuilla.'
          ),
          confirmLabel: t(
            'football.tournaments.lifecycle.confirm.startGroupStage.confirm',
            'Aloita alkulohkot'
          ),
          variant: 'default',
      ...
      case 'startPlayoffStage':
        return {
          title: t(
            'football.tournaments.lifecycle.confirm.startPlayoffStage.title',
            'Aloita pudotuspelit'
          ),
          description: t(
            'football.tournaments.lifecycle.confirm.startPlayoffStage.description',
          ...
          confirmLabel: t(
            'football.tournaments.lifecycle.confirm.startPlayoffStage.confirm',
            'Aloita pudotuspelit'
          ),
          variant: 'default',
      ...
      case 'complete':
        return {
          title: t(
            'football.tournaments.lifecycle.confirm.complete.title',
            'Päätä turnaus'
          ),
          description: t(
            'football.tournaments.lifecycle.confirm.complete.description',
            'Sulkee turnauksen lopullisesti. Tämän jälkeen otteluita tai pisteitä ei enää muuteta.'
          ),
          confirmLabel: t(
            'football.tournaments.lifecycle.confirm.complete.confirm',
            'Päätä turnaus'
          ),
          variant: 'default',
      ...
      case 'cancel':
        return {
          title: t(
            'football.tournaments.lifecycle.confirm.cancel.title',
            'Peru turnaus'
          ),
          description: t(
            'football.tournaments.lifecycle.confirm.cancel.description',
          ...
          confirmLabel: t(
            'football.tournaments.lifecycle.confirm.cancel.confirm',
            'Kyllä, peru turnaus'
          ),
          variant: 'destructive',
          destructiveAcknowledge: t(
            'football.tournaments.lifecycle.confirm.cancel.acknowledge',
    ...
    <div className="tlb">
      <div className="tlb-status-row">
        <div className="tlb-status-pair">
          <span className="tlb-status-label">
            {t('football.tournaments.statusLabel', 'Tila')}:
          </span>
            ...
            {(cancelEligibility.visible || onMoreAction) && (
              <>
                <button
                  ...
                  aria-label={t(
                    'football.tournaments.lifecycle.moreActions',
                    'Lisätoiminnot'
                  )}
                  disabled={loading}
                >
                  ...
                  <div className="tlb-more-menu" role="menu">
                    {cancelEligibility.visible && (
                      ...
                      >
                        ...
                        <span>
                          {t(
                            'football.tournaments.lifecycle.cancel',
                            'Peru turnaus'
                          )}
                        </span>
                      </button>
                    ...
                    {onMoreAction && (
                      <button
                        ...
                        title={t(
                          'football.tournaments.lifecycle.tooltip.delete',
                      ...
                      >
                        ...
                        <span>
                          {t(
                            'football.tournaments.lifecycle.delete',
                            'Poista turnaus'
                          )}
                        </span>
                      </button>
      ...
      {matchesError && !matchesLoading && (
        <p className="tlb-matches-error" role="status">
          ...
          {t(
            'football.tournaments.lifecycle.matchesLoadError',
      ...
      {activeMoreAction === 'delete' && onMoreAction && (
        <TournamentLifecycleConfirmModal
          ...
          variant="destructive"
          title={t(
            'football.tournaments.lifecycle.confirm.delete.title',
            'Poista turnaus'
          )}
          description={t(
            'football.tournaments.lifecycle.confirm.delete.description',
          ...
          prerequisites={[]}
          confirmLabel={t(
            'football.tournaments.lifecycle.confirm.delete.confirm',
            'Kyllä, poista turnaus'
          )}
          destructiveAcknowledgeLabel={t(
            'football.tournaments.lifecycle.confirm.delete.acknowledge',
