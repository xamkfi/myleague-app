...
interface PlayoffScheduleManagerProps {
  ...
  onUpdated: (tournament: FootballTournamentDto) => void;
}
...
const ROUND_OPTIONS: ReadonlyArray<FootballPlayoffRoundKey> = [
  'QuarterFinal',
  'SemiFinal',
  'ThirdPlaceMatch',
  'Final',
...
const ROUND_SORT_ORDER: Record<FootballPlayoffRoundKey, number> = {
  QuarterFinal: 1,
  SemiFinal: 2,
  ThirdPlaceMatch: 3,
  Final: 4,
 ...
 */
function nextOrderForRound(slots: EditableSlot[], round: FootballPlayoffRoundKey): number {
...
export default function PlayoffScheduleManager({ tournament, onUpdated }: PlayoffScheduleManagerProps): ReactElement {
  ...
  const addSlot = useCallback((round: FootballPlayoffRoundKey): void => {
    setSlots((prev) => {
  ...
  const handleGenerateLayout = useCallback((): void => {
    if (expectedPlayoffTeams !== 2 && expectedPlayoffTeams !== 4 && expectedPlayoffTeams !== 8) {
      setError(t(
        'football.tournaments.playoffScheduleManager.errors.unsupportedSize',
    ...
    const target: Array<{ round: FootballPlayoffRoundKey; order: number }> = [];
  ...
  const handleSave = useCallback(async (): Promise<void> => {
    ...
    for (const s of slots) {
      ...
      if (!s.scheduledDateTime) {
        setError(t(
          'football.tournaments.playoffScheduleManager.errors.missingTime',
    ...
    if (conflicts.length > 0) {
      setError(t(
        'football.tournaments.playoffScheduleManager.errors.duplicateSlot',
    ...
    try {
      setSaving(true);
      const response = await footballTournamentService.updatePlayoffSchedule(tournament.id, payload);
      if (response.data) {
        onUpdated(response.data);
        setSuccessMessage(t(
          'football.tournaments.playoffScheduleManager.savedSuccess',
      ...
      <div className="psm psm--disabled">
        <div className="psm__header">
          <h4 className="psm__title">
            ...
            {t('football.tournaments.playoffScheduleManager.title', 'Planned playoff schedule')}
          </h4>
        </div>
        <p className="psm__hint">
          {t(
            'football.tournaments.playoffScheduleManager.disabledHint',
  ...
  return (
    <div className="psm">
        ...
        {!isLocked && (
          <div className="psm__actions">
            <button
              ...
              disabled={saving}
              title={t(
                'football.tournaments.playoffScheduleManager.fillSuggestedTooltip',
                'Add the missing bracket slots based on the configured tournament rules.'
              )}
            >
              <i className="fas fa-magic" aria-hidden="true" />{' '}
              {t('football.tournaments.playoffScheduleManager.fillSuggested', 'Fill in missing slots')}
            </button>
            <button
              type="button"
      ...
      <p className="psm__hint">
        {isLocked
          ? t(
              'football.tournaments.playoffScheduleManager.lockedHint',
          ...
          : t(
              'football.tournaments.playoffScheduleManager.hint',
      ...
      {sortedSlots.length === 0 ? (
        <div className="psm__empty">
          <p>
            {t(
              'football.tournaments.playoffScheduleManager.empty',
        ...
        <div className="psm__table-wrap">
          <table className="psm__table">
            <thead>
              <tr>
                <th>{t('football.tournaments.playoffScheduleManager.fields.round', 'Round')}</th>
                <th>{t('football.tournaments.playoffScheduleManager.fields.order', 'Order')}</th>
                <th>{t('football.tournaments.playoffScheduleManager.fields.scheduledDateTime', 'Date & time')}</th>
                <th>{t('football.tournaments.playoffScheduleManager.fields.venue', 'Venue (optional)')}</th>
                {!isLocked && (
                ...
                <tr key={slot.key}>
                  <td>
                    <select
                      ...
                      onChange={(e: ChangeEvent<HTMLSelectElement>): void => {
                        const round = e.target.value as FootballPlayoffRoundKey;
                        updateSlot(slot.key, { round });
                      }}
                    >
                      {ROUND_OPTIONS.map((r) => (
                        <option key={r} value={r}>
                          {t(`football.tournaments.playoffScheduleManager.rounds.${r}`, r)}
                        </option>
                      ))}
                    </select>
                  </td>
                  ...
                  <td>
                    <input
                      ...
                      value={slot.venue}
                      placeholder={t(
                        'football.tournaments.playoffScheduleManager.placeholders.venue',
                        'e.g. Court 1'
                      )}
                      disabled={isLocked || saving}
                      onChange={(e: ChangeEvent<HTMLInputElement>): void => {
                  ...
                  {!isLocked && (
                    <td className="psm__table-actions-col">
                      <button
                        ...
                        disabled={saving}
                        aria-label={t(
                          'football.tournaments.playoffScheduleManager.removeRowAriaLabel',
                          'Poista tämä pudotuspelipaikka'
                        )}
                        title={t(
                          'football.tournaments.playoffScheduleManager.removeRowTooltip',
        ...
        <div className="psm__add-row">
          {ROUND_OPTIONS.map((round) => (
            ...
            >
              ...
              {t(`football.tournaments.playoffScheduleManager.addRound.${round}`, `Add ${round}`)}
            </button>
          ))}
        </div>
      )}
