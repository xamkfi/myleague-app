...
interface MatchFormProps {
  mode: MatchFormMode;
  initialData?: FootballMatchDto;
  onSubmit: (matchData: CreateFootballMatchRequest) => Promise<void>;
  onCancel: () => void;
...
const MatchForm = ({
  ...
  const competitionLabel: string = isTournamentMatch
    ? t('football.matches.matchForm.tournament', 'Turnaus')
    : t('football.matches.matchForm.season', 'Kausi');

  const competitionPlaceholder: string = isTournamentMatch
    ? t('football.matches.matchForm.selectTournament', 'Valitse turnaus')
    : t('football.matches.matchForm.selectSeason', 'Valitse kausi');

  const competitionSearchPlaceholder: string = isTournamentMatch
    ? t('football.matches.matchForm.searchTournaments', 'Hae turnauksia...')
    : t('football.matches.matchForm.searchSeasons', 'Hae kausia...');

  const competitionEmptyMessage: string = isTournamentMatch
    ? t('football.matches.matchForm.noTournaments', 'Turnauksia ei löytynyt')
    : t('football.matches.matchForm.noSeasons', 'Kausia ei löytynyt');

  const [formData, setFormData] = useState<CreateFootballMatchRequest>({
    ...
    if (mode === 'edit' && initialData) {
      try {
        ...
        const competitionResult = isTournamentMatch
          ? await footballTournamentSearchService.searchTournaments('', 1)
          : await footballSeasonSearchService.searchSeasons('', 1);
        ...
        const homeTeamResult = await footballTeamNameSearchService.searchTeams('', 1);
        ...
        const awayTeamResult = await footballTeamNameSearchService.searchTeams('', 1);
  ...
  const searchCompetitionsWithInitial = useCallback(
    async (query: string, page: number) => {
      ...
      const result = isTournamentMatch
        ? await footballTournamentSearchService.searchTournaments(query, page)
        : await footballSeasonSearchService.searchSeasons(query, page);
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="modal-form">
        ...
        */}
        <div className="form-group create-match-form-row">
          <label htmlFor="homeTeam">{t('football.matches.homeTeamLabel', 'Home Team')}</label>
          <div className="input-wrapper">
            <SearchableInfiniteDropdown
              placeholder={t('football.matches.homeTeamPlaceholder', 'Select Home Team (optional)')}
        ...
        <div className="form-group create-match-form-row">
          <label htmlFor="awayTeam">{t('football.matches.awayTeamLabel', 'Away Team')}</label>
          <div className="input-wrapper">
            <SearchableInfiniteDropdown
              placeholder={t('football.matches.awayTeamPlaceholder', 'Select Away Team (optional)')}
        ...
        <div className="form-help-text">
          {t(
            'football.matches.teamsOptionalHint',
        ...
        <div className="form-actions">
          ...
          {mode === 'edit' &&
            ...
            onReactivateMatch && (
              ...
              >
                {reactivateLoading
                  ? t('football.matches.reactivating', 'Reactivating...')
                  : t('football.matches.actions.reactivate', 'Reactivate Match')}
              </button>
            )}
          ...
          {mode === 'edit' &&
            ...
            initialData.status !== 'Completed' && (
              ...
              >
                {cancelLoading
                  ? t('football.matches.cancelling', 'Cancelling...')
                  : t('football.matches.actions.cancel', 'Cancel Match')}
              </button>
            )}
          ...
          <button type="submit" disabled={loading} className="submit-button">
            {loading
              ? mode === 'create'
                ? t('football.matches.creating', 'Creating...')
                : t('football.matches.updating', 'Updating...')
              : mode === 'create'
                ? t('football.matches.createMatch', 'Create Match')
                : t('football.matches.updateMatch', 'Update Match')}
          </button>
        </div>
      </form>
      ...
      <ConfirmationDialog
        ...
        icon="⚠️"
        title={t('football.matches.confirmCancel.title', 'Cancel Match')}
        message={t(
          'football.matches.confirmCancel.message',
          'Are you sure you want to cancel this match? This will mark the match as cancelled.'
        )}
        confirmText={t('football.matches.confirmCancel.confirm', 'Yes, Cancel Match')}
      ...
      <ConfirmationDialog
        ...
        icon="✅"
        title={t('football.matches.confirmReactivate.title', 'Reactivate Match')}
        message={t(
          'football.matches.confirmReactivate.message',
          'Are you sure you want to reactivate this match? This will set the match back to Scheduled status.'
        )}
        confirmText={t('football.matches.confirmReactivate.confirm', 'Yes, Reactivate Match')}
        cancelText={t('common.cancel', 'Cancel')}
