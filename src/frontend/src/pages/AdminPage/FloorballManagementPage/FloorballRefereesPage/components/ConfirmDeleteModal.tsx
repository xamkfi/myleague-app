...
interface ConfirmDeleteModalProps {
  isOpen: boolean;
  referee: FloorballRefereeDto | null;
  onConfirm: () => void;
  onCancel: () => void;
  isDeleting: boolean;
}
    ...
    <div className="modal-overlay" onClick={onCancel}>
      <div className="confirm-delete-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h3>{t('floorball.referees.confirmDelete.title', 'Confirm Delete')}</h3>
        </div>
        
        <div className="modal-body">
          <p className="warning-text">
            {t('floorball.referees.confirmDelete.message', 
          ...
          <div className="referee-details">
            <div className="detail-item">
              <span className="label">{t('floorball.referees.table.name', 'Name')}:</span>
              <span className="value">{referee.person.fullName}</span>
            </div>
            <div className="detail-item">
              <span className="label">{t('floorball.referees.table.status', 'Status')}:</span>
            ...
            <div className="detail-item">
              <span className="label">{t('floorball.referees.table.matchesOfficiated', 'Matches Officiated')}:</span>
              <span className="value">{referee.matchesOfficiated}</span>
            </div>
            {referee.licenseExpiryDate && (
              <div className="detail-item">
                <span className="label">{t('floorball.referees.table.licenseExpiry', 'License Expiry')}:</span>
