...
function getMatchStatusClass(status: FloorballMatchStatus): string {
  switch (status) {
    case FloorballMatchStatus.Completed: return 'schedule-match--completed';
    case FloorballMatchStatus.InProgress: return 'schedule-match--live';
    case FloorballMatchStatus.Cancelled: return 'schedule-match--cancelled';
    case FloorballMatchStatus.Postponed: return 'schedule-match--postponed';
    default: return 'schedule-match--scheduled';
  }
}

function getStatusLabel(status: FloorballMatchStatus, t: (key: string) => string): string {
  switch (status) {
    case FloorballMatchStatus.Completed: return t('fixtures.statusCompleted');
    case FloorballMatchStatus.InProgress: return t('fixtures.statusLive');
    case FloorballMatchStatus.Cancelled: return t('fixtures.statusCancelled');
    case FloorballMatchStatus.Postponed: return t('fixtures.statusPostponed');
    default: return '';
  }
}
...
export default function FixturesSection({
  ...
  const filteredMatches = useMemo(() => {
    ...
    switch (filter) {
      case 'upcoming':
        return matches.filter(m => m.status !== FloorballMatchStatus.Completed);
      case 'past':
        return matches.filter(m => m.status === FloorballMatchStatus.Completed);
      case 'next': {
        const nextMatch = matches.find(m =>
          m.status === FloorballMatchStatus.Scheduled || m.status === FloorballMatchStatus.InProgress
        );
