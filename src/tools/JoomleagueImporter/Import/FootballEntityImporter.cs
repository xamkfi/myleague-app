...
public class FootballEntityImporter
{
    ...
    public FootballEntityImporter(ApiClient api, IdMapStore idMap, ImportLogger log)
    {
        _api = api;
        _idMap = idMap;
        _log = log;
    ...
    public async Task ImportPersonsAndPlayersAsync(FloorballImportSet set)
    {
        ...
        List<FootballPlayerDto> existingPlayers = await _api.GetFootballPlayersAsync();
        ...
        foreach (OldPerson oldPerson in set.UniquePersons.Values)
        {
            ...
            if (player == null)
            {
                player = await _api.CreateFootballPlayerAsync(person.Id);
                if (player == null)
                {
    ...
    public async Task ImportTeamsAsync(FloorballImportSet set, JoomleagueDatabase db, DivisionDto division)
    {
        ...
        List<Application.Features.Football.Teams.DTOs.FootballTeamSummaryDto> existing = await _api.GetFootballTeamsAsync();
        int created = 0, reused = 0;
        ...
        foreach (OldTeam oldTeam in set.UniqueTeams.Values)
        {
            ...
            if (team == null)
            {
                ...
                if (team == null)
                {
                    ...
                    FootballTeamDto? createdTeam = await _api.CreateFootballTeamAsync(
            ...
            foreach (RosterEntry re in unionRoster.Values)
            {
                ...
                bool ok = await _api.AddFootballPlayerToTeamAsync(team.Id, mapping.PlayerId, position, jersey);
                if (ok) added++;
            }
    ...
    public async Task<Guid> GetOrCreateImportRefereeAsync()
    {
        ...
        List<FootballRefereeDto> existing = await _api.GetFootballRefereesAsync();
        if (existing.Count > 0)
        {
        ...
        FootballRefereeDto? referee = await _api.CreateFootballRefereeAsync(refPerson.Id);
        if (referee == null)
        ...
        bool cachePrimary)
    {
        ...
        FootballPlayerDto? player = await _api.CreateFootballPlayerAsync(person.Id);
        if (player == null)
        {
            List<FootballPlayerDto> players = await _api.GetFootballPlayersAsync();
        ...
        bool added = await _api.AddFootballPlayerToTeamAsync(
    ...
    public async Task<FootballSeasonDto?> ImportSeasonAsync(ProjectImport pi, DivisionDto division)
    {
        ...
        List<FootballSeasonDto> existing = await _api.GetFootballSeasonsAsync();
        ...
        if (season == null)
        {
            ...
            season = await _api.CreateFootballSeasonAsync(
        ...
        TeamCategory expected)
    {
        ...
        FootballSeasonDto? updated = await _api.UpdateFootballSeasonAsync(season, expected);
        if (updated == null)
        {
            _log.LogError(
    ...
    private async Task EnsureTeamsInSeasonAsync(ProjectImport pi, FootballSeasonDto season, DivisionDto division)
    {
        ...
        foreach (ProjectTeamImport pti in pi.Teams.Values)
        {
            ...
            if (!handled.Add(teamId))
                continue;
            bool ok = await _api.AddTeamToFootballSeasonAsync(season.Id, teamId);
            await _api.AddTeamToFootballSeasonDivisionAsync(season.Id, division.Id, teamId);
            if (ok) teamsAdded++;
        }
