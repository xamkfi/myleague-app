...
Imports **floorball**, **football**, or **hockey** data from a JoomLeague MySQL dump (`.sql`) into a running WebAPI. It parses clubs, teams, persons, projects (seasons), matches, match events, and `jos_joomleague_match_player` appearances, then posts them in order. Games-played statistics are created only for players who have a match appearance or a match event — the season roster is not treated as having played every game. When a match has no appearance rows, only event participants (plus unknown pads needed to start the match) are imported. Season intro cards come from project/season text columns (`description`, `projectinfo`, `extended`, `extension`) when the dump has them; otherwise the importer stores a single “History data” card. An on-disk id map makes re-runs idempotent. After each season’s matches the importer marks those competition roster rows inactive (`IsActive=false` / hockey `RosterStatus=Inactive`) so dump data stays historical, not a current club assignment. Hockey seasons are completed after recalculation. Expected season standings and scoring are written to `reports/expected-{sport}.json` from the dump (3/1/0 points); after import `reports/compare-{sport}.json` diffs that file against the API.
