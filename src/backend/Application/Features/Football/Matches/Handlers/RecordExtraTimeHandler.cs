...
public class RecordExtraTimeHandler : IRequestHandler<RecordExtraTimeCommand, Result<FootballMatchDto>>
{
    ...
    private readonly ILogger<RecordExtraTimeHandler> _logger;

    public RecordExtraTimeHandler(
        IFootballMatchRepository matchRepository,
        IFootballUnitOfWork unitOfWork,
        ILogger<RecordExtraTimeHandler> logger)
    {
    ...
    public async Task<Result<FootballMatchDto>> Handle(RecordExtraTimeCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Error occurred while recording extra time for match {MatchId}", request.MatchId);
            return Result<FootballMatchDto>.Failure("An error occurred while recording extra time.");
        }
    }
}
