...
namespace MyLeague.Infrastructure.DomainEvents.Handlers.Floorball
{
    ...
    public class FloorballMatchShootoutStartedEventHandler : SignalRDomainEventHandler<FloorballMatchShootoutStartedEvent>
    {
        ...
        /// <summary>
        /// Processes the FloorballMatchShootoutStartedEvent before notification
        /// </summary>
        /// <param name="domainEvent">The domain event to process</param>
        /// <returns>A task representing the asynchronous operation</returns>
        protected override async Task ProcessEventAsync(FloorballMatchShootoutStartedEvent domainEvent)
        {
            ...
            if (match == null)
            {
                _logger.LogWarning("Floorball match with ID {MatchId} not found for ShootoutStarted event.", domainEvent.MatchId);
                return;
            }
            ...
            _logger.LogInformation("Shootout started in match between {HomeTeam} and {AwayTeam}", homeTeamName, awayTeamName);

            await NotifyAsync("FloorballMatchShootoutStarted", payload);
        }
    }
} 