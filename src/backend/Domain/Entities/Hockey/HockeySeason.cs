...
public class HockeySeason : BaseEntity
{
    ...
    /// </summary>
    public IReadOnlyCollection<HockeyTeam> Teams => _teams.AsReadOnly();
    private readonly List<HockeyTeam> _teams = new();

    /// <summary>
    /// Gets the matches scheduled for this season
    /// </summary>
    public IReadOnlyCollection<HockeyMatch> Matches => _matches.AsReadOnly();
    private readonly List<HockeyMatch> _matches = new();

    /// <summary>
    /// Private constructor for EF Core
    /// </summary>
    private HockeySeason()
    {
        Id = Guid.NewGuid();
        ...
        IsActive = false;
        IsCompleted = false;
        _teams = new List<HockeyTeam>();
        _matches = new List<HockeyMatch>();
    }

    /// <summary>
    /// Initializes a new instance of the HockeySeason class
    /// </summary>
    ...
    public HockeySeason(string name, HockeyDivision division, DateTime startDate, DateTime endDate)
    {
    ...
    public void AddTeam(HockeyTeam team)
    {
    ...
    /// <exception cref="ArgumentNullException">Thrown when the team is null</exception>
    /// <exception cref="InvalidOperationException">Thrown when the season is completed</exception>
    public void RemoveTeam(HockeyTeam team)
    {
        ArgumentNullException.ThrowIfNull(team);
        if (IsCompleted)
